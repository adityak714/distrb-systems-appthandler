...
async function mailBookingConfirmation(recipient: string, dentistry: string, date: Date, name: String) {
...
mailBookingConfirmation(name, recipient, dentistry, date).catch(console.error);


// Booking change email, sent when a user changes an appointment.

async function mailBookingChange(recipient: string, dentistry: string, date: Date, name: String) {
  await nodemailer.createTestAccount();
  const transporter = nodemailer.createTransport({
    host: 'smtp.office365.com',
    port: 587,
...
async function mailBookingDeletion(recipient: string, dentistry: string, date: Date, name: string) {
  await nodemailer.createTestAccount();
  const transporter = nodemailer.createTransport({
    host: 'smtp.office365.com',
    port: 587,
